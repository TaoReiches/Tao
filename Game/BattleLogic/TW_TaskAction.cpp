...
void BeTaskActionAttack::SetTargetItem(int iItemID)
{
	gUnit.SetAttackingUnitID(0);
	if (m_iItemID != iItemID)
	{
		m_iItemID = iItemID;

		const BeMapItem* pkTarget = gMapItemMgr.GetMapItemByID(m_iItemID);
		if (pkTarget && gUnit.GetClass() != UNIT_CLASSTYPE_BUILDING)
		{
			float fTarFace = atan2f(pkTarget->GetPosY() - gUnit.GetPosY(), pkTarget->GetPosX() - gUnit.GetPosX());
			gUnit.SetTarFace(fTarFace);
		}
...
BeExeResult BeTaskActionAttack::Execute(int& iDeltaTime)
{
	...
	if (!m_bAttacking)
	{
		...
		if (pkTarget)
		{
			...
			m_kAttr = kAttr;
			m_kAttr.SetFlag(BAF_FIRST);
			m_kAttr.SetFlag(BAF_NORMAL);
